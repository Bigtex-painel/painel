...
# BASES_DIR é setado no modo Actions (--bases-dir). Quando None, usa PROJECT_DIR (modo legado).
BASES_DIR = None
...
# --------------------------------------------------------------------------------------
# Identificação de arquivos por CONTEÚDO (não por nome)
# --------------------------------------------------------------------------------------
# Usado no modo Actions: o ERP exporta xls com nome aleatório, e precisamos
# identificar (tipo, ano) só olhando dentro do arquivo.

# Schemas distintivos validados em 2022-2026 (idênticos em todos os anos):
SCHEMA_VENDAS = {'Resultado', 'Vlr.Custo', 'Descrição do Produto', 'Vlr.Total', 'Qtde.', 'Vlr.Tot.Custo'}
SCHEMA_PAGAR = {'FORNECEDOR', 'DT.EMISSÃO', 'DT.VENC.', 'VLR.PARCELA', 'SUB-CONTA'}
SCHEMA_RECEBER = {'CLIENTE', 'Nº NFE', '% CF', 'VLR.LIQUIDO', 'DT.EMISSÃO'}

COLUNA_DATA_POR_TIPO = {
    'vendas': 'Dt.Emissão',
    'pagar': 'DT.EMISSÃO',
    'receber': 'DT.EMISSÃO',
}

def identificar_xls(path: Path) -> tuple:
    """Identifica (tipo, ano, erro) de um xls pelo conteúdo.

    Lê apenas a primeira linha pra detectar o tipo via schema.
    Depois lê o arquivo todo pra extrair o ano predominante das datas.

    Retorna:
      (tipo, ano, None)              se sucesso
      (None, None, 'mensagem_erro')  se não reconhecer
    """
    try:
        df_head = pd.read_excel(path, sheet_name=0, nrows=1)
        cols = {str(c).strip() for c in df_head.columns}
    except Exception as e:
        return None, None, f'erro ao ler xls: {e}'

    if SCHEMA_VENDAS.issubset(cols):
        tipo = 'vendas'
    elif SCHEMA_PAGAR.issubset(cols) and 'FORNECEDOR' in cols:
        tipo = 'pagar'
    elif SCHEMA_RECEBER.issubset(cols) and 'CLIENTE' in cols and 'Nº NFE' in cols:
        tipo = 'receber'
    else:
        return None, None, f'schema desconhecido (colunas: {sorted(cols)[:6]}...)'

    # Ler arquivo completo pra extrair ano predominante
    try:
        df_full = pd.read_excel(path, sheet_name=0)
        col_data = COLUNA_DATA_POR_TIPO[tipo]
        if col_data not in df_full.columns:
            return tipo, None, f'tipo {tipo} mas coluna "{col_data}" ausente'
        anos_serie = pd.to_datetime(df_full[col_data], errors='coerce').dt.year.dropna()
        if anos_serie.empty:
            return tipo, None, 'sem datas válidas na coluna de emissão'
        ano = int(anos_serie.astype(int).mode().iloc[0])
        return tipo, ano, None
    except Exception as e:
        return tipo, None, f'erro ao extrair ano: {e}'

def nome_canonico_xls(tipo: str, ano: int) -> str:
    """Nome padronizado em BASES/ — ex: 'vendas_2026.xlsx'."""
    return f'{tipo}_{ano}.xlsx'

def nome_canonico_pdf(banco: str) -> str:
    """Nome padronizado em BASES/ — ex: 'extrato_itau.pdf'.
    Normaliza o nome do banco removendo acentos e espaços."""
    norm = (banco or 'desconhecido').lower()
    norm = norm.replace('ú', 'u').replace('ç', 'c').replace('ã', 'a').replace('á', 'a')
    norm = norm.replace(' ', '_')
    return f'extrato_{norm}.pdf'
...
# Cache do índice de BASES — calculado uma vez, reutilizado entre chamadas
_BASES_INDEX = None

def _indexar_bases(bases_dir: Path) -> dict:
    """Varre BASES e indexa xls por (tipo, ano).

    Aceita arquivos com nome canônico ('vendas_2026.xlsx') e também
    arquivos com nome diferente (identifica por conteúdo como fallback).
    Cache em memória por execução.
    """
    global _BASES_INDEX
    if _BASES_INDEX is not None:
        return _BASES_INDEX
    index = {}
    for xlsx in sorted(bases_dir.glob('*.xlsx')) + sorted(bases_dir.glob('*.XLSX')):
        # 1ª tentativa: nome canônico
        m = re.match(r'^(vendas|pagar|receber)_(\d{4})\.xlsx$', xlsx.name, re.IGNORECASE)
        if m:
            tipo = m.group(1).lower()
            ano = int(m.group(2))
            index.setdefault((tipo, ano), xlsx)
            continue
        # 2ª tentativa: identificar por conteúdo (caso o usuário tenha colocado um xls "cru")
        tipo, ano, _err = identificar_xls(xlsx)
        if tipo and ano:
            index.setdefault((tipo, ano), xlsx)
    _BASES_INDEX = index
    return index
...
def _path_xls(tipo: str, ano: int) -> Path:
    """Resolve o path de um xls (tipo, ano).

    Modo BASES (quando BASES_DIR é setado): procura nome canônico
    '<tipo>_<ano>.xlsx' em BASES_DIR. Fallback: identifica por conteúdo.

    Modo LEGADO (BASES_DIR=None): matching tolerante de nome em PROJECT_DIR
    (mantém compatibilidade com o uso local atual).
    """
    # =========== Modo BASES (Actions / produção) ===========
    if BASES_DIR is not None:
        index = _indexar_bases(BASES_DIR)
        if (tipo, ano) in index:
            return index[(tipo, ano)]
        raise FileNotFoundError(
            f'arquivo {nome_canonico_xls(tipo, ano)} não encontrado em {BASES_DIR} '
            f'(arquivos disponíveis: {sorted(index.keys())})'
        )

    # =========== Modo LEGADO (matching de nome) ===========
    sufixos = {
...
def _anos_disponiveis() -> list:
    """Retorna anos disponíveis em BASES (modo Actions) ou ANOS hardcoded (modo legado).

    Em produção (modo BASES), os anos são descobertos dinamicamente pelo índice.
    Isso permite virar de ano (2027+) sem mexer no código.
    """
    if BASES_DIR is not None:
        index = _indexar_bases(BASES_DIR)
        anos = sorted({ano for (_tipo, ano) in index.keys()})
        if anos:
            return anos
    return list(ANOS)

def carregar_pagar(anos=None) -> pd.DataFrame:
    """Carrega e normaliza CONTAS_A_PAGAR de todos os anos."""
    if anos is None:
        anos = _anos_disponiveis()
    dfs = []
    for ano in anos:
        sh = SHEETS_PAGAR.get(ano, 'Plan1')  # default Plan1 pra anos novos
...
def carregar_receber(anos=None) -> pd.DataFrame:
    """Carrega e normaliza CONTAS_A_RECEBER de todos os anos."""
    if anos is None:
        anos = _anos_disponiveis()
    dfs = []
    for ano in anos:
        sh = SHEETS_RECEBER.get(ano, 'Plan1')
...
def carregar_vendas(anos=None) -> pd.DataFrame:
...
# --------------------------------------------------------------------------------------
# Processamento INBOX → BASES (modo Actions)
# --------------------------------------------------------------------------------------

def processar_inbox(inbox_dir: Path, bases_dir: Path,
                    processados_dir: Path = None) -> dict:
    """Lê arquivos do INBOX, identifica cada um por conteúdo,
    sobrescreve em BASES com nome canônico, e arquiva em PROCESSADOS.

    Para cada arquivo:
      - .xlsx/.xls: identifica (tipo, ano) → BASES/<tipo>_<ano>.xlsx
      - .pdf:       identifica banco       → BASES/extrato_<banco>.pdf
      - outros:     ignora com aviso

    Após processado com sucesso:
      - Copia pra PROCESSADOS/<arquivo_original> (auditoria, se setado)
      - Remove do INBOX

    Em caso de erro: arquivo permanece no INBOX e o erro é registrado.

    Retorna dict {processados, avisos, erros}.
    """
    bases_dir.mkdir(parents=True, exist_ok=True)
    if processados_dir:
        processados_dir.mkdir(parents=True, exist_ok=True)

    relatorio = {'processados': [], 'avisos': [], 'erros': []}

    if not inbox_dir.exists():
        relatorio['erros'].append(f'INBOX não existe: {inbox_dir}')
        return relatorio

    arquivos = sorted([p for p in inbox_dir.iterdir() if p.is_file()])
    if not arquivos:
        relatorio['avisos'].append('INBOX vazio — nenhum arquivo novo para processar')
        return relatorio
    ...
    for arq in arquivos:
        ...
        try:
            if ext in ('.xlsx', '.xls'):
                tipo, ano, err = identificar_xls(arq)
                if not tipo:
                    relatorio['erros'].append(
                        f'{arq.name}: schema desconhecido — {err}'
                    )
                    continue
                if not ano:
                    relatorio['erros'].append(
                        f'{arq.name}: tipo {tipo} mas {err}'
                    )
                    continue
                destino = bases_dir / nome_canonico_xls(tipo, ano)
                sobrescreveu = destino.exists()
                shutil.copy2(arq, destino)
                if processados_dir:
                    shutil.copy2(arq, processados_dir / arq.name)
                arq.unlink()
                relatorio['processados'].append({
                    'origem': arq.name,
                ...
                banco, saldo, data = _extrair_dados_pdf_extrato(arq)
                if not banco:
                    relatorio['erros'].append(
                        f'{arq.name}: PDF não é extrato bancário reconhecido'
                    )
                    continue
                if saldo is None:
                    relatorio['avisos'].append(
                        f'{arq.name}: extrato {banco} sem saldo extraível (Sld. Final ausente)'
                    )
                destino = bases_dir / nome_canonico_pdf(banco)
                sobrescreveu = destino.exists()
                shutil.copy2(arq, destino)
                if processados_dir:
                    shutil.copy2(arq, processados_dir / arq.name)
                ...
                relatorio['avisos'].append(f'{arq.name}: extensão {ext} ignorada')

        except Exception as e:
            relatorio['erros'].append(f'{arq.name}: erro inesperado — {type(e).__name__}: {e}')

    # Após processar, invalidar cache do índice de BASES (arquivos novos chegaram)
    global _BASES_INDEX
    _BASES_INDEX = None
...
def validar_bases(bases_dir: Path, ano_corrente: int) -> dict:
    """Verifica se BASES tem o mínimo necessário pro gerar_payload rodar.

    Retorna {ok: bool, avisos: [...], erros: [...]}.
    """
    res = {'ok': True, 'avisos': [], 'erros': []}
    if not bases_dir.exists():
        res['erros'].append(f'BASES não existe: {bases_dir}')
        res['ok'] = False
        return res

    index = _indexar_bases(bases_dir)

    # Mínimo: 3 arquivos do ano corrente
    for tipo in ('vendas', 'pagar', 'receber'):
        if (tipo, ano_corrente) not in index:
            res['erros'].append(
                f'falta {tipo}_{ano_corrente}.xlsx em BASES (necessário pro ano corrente)'
            )
            res['ok'] = False

    # Avisar se faltam históricos (não bloqueia, mas pode degradar análise)
    anos_pres = sorted({a for (_, a) in index.keys()})
    if anos_pres and len(anos_pres) < 3:
        res['avisos'].append(
            f'apenas {len(anos_pres)} ano(s) histórico(s) em BASES: {anos_pres}'
        )

    # PDFs de extrato — não obrigatórios, mas avisar se faltarem
    pdfs = list(bases_dir.glob('extrato_*.pdf'))
    if not pdfs:
        res['avisos'].append('nenhum PDF de extrato em BASES — saldos virão do fallback')

    return res


# --------------------------------------------------------------------------------------
# Status estruturado (lido pelo GitHub Actions)
# --------------------------------------------------------------------------------------

class StatusReport:
    """Coleta resultado da execução em formato estruturado pra Actions/notificação."""

    def __init__(self):
        self.fase = 'inicio'
        self.processados = []      # ops INBOX→BASES bem-sucedidas
        self.avisos = []           # situações esperadas mas que merecem atenção
        self.erros = []            # falhas que impediram trabalho parcial ou total
        self.payloads_gerados = [] # paths dos payloads escritos
        self.metadata = {
            'data_ref': None,
            'data_extratos': None,
            'inicio': datetime.now().isoformat(timespec='seconds'),
            'fim': None,
            'host': os.uname().nodename if hasattr(os, 'uname') else 'desconhecido',
        }
    ...
    def absorver_relatorio(self, rel: dict):
        """Absorve o dict {processados, avisos, erros} de processar_inbox/validar_bases."""
        self.processados.extend(rel.get('processados', []))
        for a in rel.get('avisos', []):
            self.aviso(a)
    ...
    def exit_code(self) -> int:
        if not self.erros:
            return 0
        # Falha total: nada foi gerado
        if not self.payloads_gerados:
            return 2
        # Erros parciais: gerou alguma coisa mas teve problema
        return 1

    def fechar(self) -> dict:
        self.metadata['fim'] = datetime.now().isoformat(timespec='seconds')
        return {
            ...
            'avisos': self.avisos,
            'erros': self.erros,
            'payloads_gerados': self.payloads_gerados,
            'metadata': self.metadata,
        }

    def gravar(self, path: str):
        Path(path).parent.mkdir(parents=True, exist_ok=True)
        with open(path, 'w', encoding='utf-8') as f:
            json.dump(self.fechar(), f, indent=2, ensure_ascii=False)
...
MODO LEGADO (rodar local com xls em uma pasta única):
  python gerar_payload.py --gerar
  python gerar_payload.py --gerar --data 2026-05-01
  python gerar_payload.py --validar --bloco tudo

MODO ACTIONS (INBOX → BASES → payloads, com status JSON):
  python gerar_payload.py --gerar \\
    --inbox-dir ./inbox \\
    --bases-dir ./bases \\
    --processados-dir ./processados/2026-05-04 \\
...
''')
    ...
    parser.add_argument('--saldos', default=None,
                        help='Caminho para arquivo JSON com saldos bancários. '
                             'Sobrescreve PDFs se passado.')
    parser.add_argument('--xls-dir', default='/mnt/project',
                        help='[Modo legado] Pasta com xls (nome contém ano). '
                             'No modo Actions, use --bases-dir.')
    parser.add_argument('--data', default=None,
                        help='Data de referência (formato YYYY-MM-DD). '
                             'Default: data dos PDFs ou DATA_REF do código.')

    # Args do modo Actions
    parser.add_argument('--inbox-dir', default=None,
                        help='[Modo Actions] Pasta INBOX (arquivos novos do dia). '
                             'Quando setado, ativa modo Actions: identifica arquivos por '
                             'conteúdo, move pra BASES, gera payloads, grava status JSON.')
    parser.add_argument('--bases-dir', default=None,
                        help='[Modo Actions] Pasta BASES (estado consolidado). '
                             'Default: --xls-dir')
    parser.add_argument('--processados-dir', default=None,
                        help='[Modo Actions] Pasta para arquivar arquivos processados '
                             '(auditoria). Opcional.')
    parser.add_argument('--status-json', default=None,
                        help='[Modo Actions] Caminho pra gravar JSON de status estruturado. '
                             'Sempre gravado, mesmo em caso de erro.')

    args = parser.parse_args()

    status = StatusReport()
    global DATA_REF, HOJE, DATA_REF_BR, ANOS, PROJECT_DIR, BASES_DIR, SALDOS_BANCARIOS

    # =====================================================================
    # Bootstrap geral (try/except externo garante que status JSON seja gravado)
    # =====================================================================
    try:
        # ----- Determinar modo -----
        modo_actions = bool(args.inbox_dir)
        if modo_actions:
            status.fase = 'setup'
            status.info(f'🚀 Modo ACTIONS ativado')
            inbox_dir = Path(args.inbox_dir)
            bases_dir = Path(args.bases_dir or args.xls_dir)
            processados_dir = Path(args.processados_dir) if args.processados_dir else None

            if not bases_dir.exists():
                status.info(f'criando BASES: {bases_dir}')
                bases_dir.mkdir(parents=True, exist_ok=True)

            BASES_DIR = bases_dir
            PROJECT_DIR = bases_dir  # PDFs e xls vêm de BASES
            status.info(f'BASES: {bases_dir}')
            status.info(f'INBOX: {inbox_dir}')
            if processados_dir:
                status.info(f'PROCESSADOS: {processados_dir}')

            # ----- Processar INBOX → BASES -----
            status.fase = 'processar_inbox'
            print()
            status.info('=== Processando INBOX → BASES ===')
            rel = processar_inbox(inbox_dir, bases_dir, processados_dir)
            status.absorver_relatorio(rel)
            for op in rel['processados']:
                if 'tipo' in op:
                    sob = ' (sobrescreveu)' if op.get('sobrescreveu') else ''
                    status.info(
                        f'  ✅ {op["origem"]} → {op["destino"]} '
                        f'[{op["tipo"]} {op["ano"]}]{sob}'
                    )
                elif 'banco' in op:
                    sob = ' (sobrescreveu)' if op.get('sobrescreveu') else ''
                    saldo_str = f'R$ {op["saldo"]:,.2f}' if op["saldo"] is not None else '(sem saldo)'
                    status.info(
                        f'  ✅ {op["origem"]} → {op["destino"]} '
                        f'[{op["banco"]}: {saldo_str}]{sob}'
                    )

            # ----- Validar BASES (tem o necessário pra rodar?) -----
            status.fase = 'validar_bases'
            ano_corrente = datetime.now().year
            if args.data:
                try:
                    ano_corrente = pd.Timestamp(args.data).year
                except Exception:
                    pass
            val = validar_bases(bases_dir, ano_corrente)
            status.absorver_relatorio({'avisos': val['avisos'], 'erros': val['erros']})
            if not val['ok']:
                status.erro('BASES não tem mínimo necessário pra gerar payload — abortando geração')
                return  # vai pro finally que grava status.json
        else:
            # ----- Modo LEGADO -----
            if args.xls_dir:
                PROJECT_DIR = Path(args.xls_dir)
                if not PROJECT_DIR.exists():
                    status.erro(f'pasta "{args.xls_dir}" não existe.')
                    return
                status.info(f'📂 Pasta de xls: {PROJECT_DIR}')

        # =====================================================================
        # Carregar saldos de PDFs (comum aos dois modos)
        # =====================================================================
        status.fase = 'carregar_saldos'
        pdf_dir = BASES_DIR if modo_actions else PROJECT_DIR
        pdf_saldos, pdf_data = carregar_saldos_de_pdfs(pdf_dir, verbose=True)
        if pdf_saldos:
            SALDOS_BANCARIOS = pdf_saldos
            status.info(f'💰 Saldos extraídos de {len(pdf_saldos)} PDF(s):')
            for b, v in sorted(pdf_saldos.items()):
                status.info(f'    {b}: R$ {v:,.2f}')
            if pdf_data and not args.data:
                try:
                    ...
                    status.info(f'📅 Usando data dos extratos: {DATA_REF_BR}')
                except Exception:
                    pass
            status.metadata['data_extratos'] = pdf_data
        else:
            status.aviso('nenhum PDF de extrato lido — usando saldos do fallback')

        # JSON de saldos sobrescreve PDFs
        if args.saldos:
            try:
                ...
                SALDOS_BANCARIOS = {k: float(v) for k, v in novos.items()}
                status.info(f'💰 Saldos sobrescritos via JSON: {SALDOS_BANCARIOS}')
            except Exception as e:
                status.erro(f'arquivo de saldos "{args.saldos}" inválido: {e}')
                return

        # --data via CLI tem prioridade máxima
        if args.data:
            try:
                dt = pd.Timestamp(args.data)
            except Exception:
                status.erro(f'data inválida "{args.data}". Use formato YYYY-MM-DD (ex: 2026-05-01).')
                return
            DATA_REF = dt
            ...
            ANOS = list(range(min(ANOS), ano_max + 1))
            status.info(f'📅 Data de referência (--data): {DATA_REF_BR} (anos: {ANOS})')

        # No modo BASES, ANOS dinâmico vem do índice
        if modo_actions:
            ANOS = _anos_disponiveis()
            status.info(f'📅 Anos disponíveis em BASES: {ANOS}')

        status.metadata['data_ref'] = HOJE
        print()
        status.info(f'data_ref = {DATA_REF.date()} | bloco = {args.bloco}')
        print()
        ...
        # =====================================================================
        if args.gerar:
            status.fase = 'gerar_payload'
            status.info('=== Gerando payload.js completo ===')
            status.info('Carregando xls (todos os anos)...')
            ...
            status.info(f'  vendas: {len(vendas_all)} | pagar: {len(pagar_all)} | receber: {len(receber_all)}')
            status.info('Gerando payload completo (43 chaves)...')
            payload = gerar_payload_completo(vendas_all, pagar_all, pagar_pre_all, receber_all)
            status.info(f'  Chaves geradas: {len(payload)}')
            escrever_payload_js(payload, args.saida)
            tamanho = os.path.getsize(args.saida)
            status.info(f'  ✓ Completo (43 chaves): {args.saida} ({tamanho:,} bytes)')
            status.payloads_gerados.append(args.saida)

            if args.saida_financeiro:
                escrever_payload_financeiro_js(payload, args.saida_financeiro)
                tamanho_fin = os.path.getsize(args.saida_financeiro)
                status.info(f'  ✓ Financeiro (9 chaves): {args.saida_financeiro} ({tamanho_fin:,} bytes)')
                status.payloads_gerados.append(args.saida_financeiro)

            status.fase = 'concluido'
            return

        # =====================================================================
        # Validação contra golden (modo desenvolvimento)
        # =====================================================================
        if args.bloco in ('mensal', 'tudo'):
            print('=== Bloco mensal ===')
            print('Carregando xls...')
            vendas = carregar_vendas([2025, 2026])
            pagar = carregar_pagar(ANOS)
        ...
        status.erro(f'arquivo não encontrado: {e}')
    except KeyError as e:
        status.erro(f'chave/coluna ausente: {e}')
    except Exception as e:
        status.erro(f'falha inesperada: {type(e).__name__}: {e}')
        import traceback
        traceback.print_exc()
    finally:
        # Sempre grava status JSON e sai com código apropriado
        if args.status_json:
            status.gravar(args.status_json)
            print(f'\n📋 Status gravado em: {args.status_json}')
        if args.inbox_dir:  # modo Actions sai com código não-zero em erro
            print(f'\n{"✅ SUCESSO" if status.sucesso else "❌ ERRO"} (exit_code={status.exit_code})')
